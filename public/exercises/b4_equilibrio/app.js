...
function updateAllUI() {
    ...
    if (avg >= 70) { status = 'Ciudad Equilibrada'; cls = 'balanced';
        // Notify parent (ArquiLab) that exercise is complete (only once)
        if (!window._b4ex1Done && window.parent && window.parent !== window) {
            window._b4ex1Done = true;
            window.parent.postMessage({ type: 'exercise-complete', exercise: 'b4_ex1' }, '*');
        }
    }
